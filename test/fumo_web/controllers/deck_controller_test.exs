 ...
 describe "index" do
  ...
  describe "create deck" do
    ...
    test "redirects to show when data is valid", %{conn: conn} do
      deck_attrs = params_with_assocs(:deck)
      conn = post(conn, Routes.deck_path(conn, :create), deck: deck_attrs)

      assert %{id: id} = redirected_params(conn)
      assert redirected_to(conn) == Routes.deck_path(conn, :show, id)

      conn = get(conn, Routes.deck_path(conn, :show, id))
      assert html_response(conn, 200) =~ "#{deck_attrs.title}</h1>"
    end
  ...
  describe "delete deck" do
    ...
    test "deletes chosen deck", %{conn: conn, deck: deck} do
      ...
      assert redirected_to(conn) == Routes.page_path(conn, :dashboard) # Routes.deck_path(conn, :index)
